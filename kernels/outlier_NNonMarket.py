...
# https://www.kaggle.com/oguzkaplan/outlier-detection1-5-iqr-method-nn-on-market?scriptVersionId=8698044
...
#data_add=market_train.iloc["time"]
#data_add.head() 
...
#a=["close"]
#for i in a:
#    b = d[6].loc[d[6][i]>=bounds[6].loc[0,i]]
#    b = d[6].loc[d[6][i]<=bounds[6].loc[1,i]]
#    print(bounds[6].loc[0,i])
#    print(bounds[6].loc[1,i])
#bounds[5].head()
#print(d[6].shape[0]-b.shape[0])
...
# df_new = {}
# for type in types:
#     filt_df = d[type].loc[:, d[type].columns != 'assetCode']
#     filt_df = filt_df.loc[:,filt_df.columns != "time"]
#     filt_df = filt_df.loc[:,filt_df.columns != "assetName"]
#     filt_df = filt_df.loc[:,filt_df.columns != "universe"]
#     filt_df = filt_df.loc[:,filt_df.columns != "returnsOpenNextMktres10"]
#     filt_df = filt_df.apply(lambda x: x[(x>bounds[type].loc[low,x.name]) & (x < bounds[type].loc[high,x.name])], axis=0)
#     df_new[type] = pd.concat([filt_df,d[type].loc[:,'assetCode']], axis=1)
# 
# 
# market_data_new = result = pd.concat(df_new)
# 
# 
# 

# market_data_new1=market_data_new1.rename(columns={'level_1':'index'})
# market_data_new1.head()

# market_train_universe=market_train["universe"]
# market_train_returnsOpenNextMktres10=market_train["returnsOpenNextMktres10"]
# market_train_time=market_train["time"]
# market_train_universe=market_train_universe.reset_index()
# market_train_returnsOpenNextMktres10=market_train_returnsOpenNextMktres10.reset_index()
# market_train_time=market_train_time.reset_index()
# market_train_universe.head()
# market_train_returnsOpenNextMktres10.head()
# market_train_time.head()

# market_train1=pd.merge(market_data_new1,market_train_universe,on='index')
# market_train2=pd.merge(market_train1,market_train_returnsOpenNextMktres10,on='index')
# market_train2=pd.merge(market_train2,market_train_time,on='index')

# market_train2["universe"].sum()
# market_train2["returnsOpenNextMktres10"].sum()
# market_train["returnsOpenNextMktres10"].sum()

# market_train2["universe"].unique()

# len(market_train2)

# In[ ]:

#market_train=pd.merge(data_add,market_data_new1,on='level_1')

#market_train = market_train.drop("assetCode", axis=1)
#market_train=market_train.drop("level_1", axis=1)

#market_train=market_train.rename(columns={'level_0':'assetCode'})
#market_train.head()
#market_data_new1.head()
...
#for type in num_cols:
#    market_train2=market_train2[~market_train2[type].isnull()]




# market_train2=market_train2.dropna()

# market_train2.head()

# market_train2.head()
# market_train2 = market_train2.drop("level_0", axis=1)
# market_train2 = market_train2.drop("index", axis=1)
# market_train2.head()
# len(market_train2)

# market_train2["assetCode"].unique()
# len(market_train2)
# market_train =market_train2.copy()
# market_train.head()

# market_train=market_train.reset_index()
# 

# market_train = market_train.drop("index", axis=1)
